...
class FrankaKinematics():
    ...
    def fwd_kin(self, joint_values):   # Expects a 7 x 1 values for joint_values
        ...
        joint_values = np.insert(joint_values, len(joint_values), 0.0, axis=0)  # added 0 at the end for the fixed joint
    ...
    def inv_kin2(self, q_current, T_desired):
        ...
        final_theta = opt.minimize(self.inv_kin_optfun, x0, method='BFGS',) # jac=self.geometric_jacobian(T_joint, T_current))
        # print 'res \n', final_theta
    ...
    def qrt_orient_error(self, theta, mu_ang_euler_des):
        ...
        orien_error = eta_e * eps_d - eta_d * eps_e - S_ed.dot(eps_e)  # eqn 3.91 Robotics Modeling Planning and Control
    ...
    def laplace_cost_and_grad_pose(self, theta, mu_theta, inv_sigma_theta, mu_x, inv_sigma_x, mu_ang_euler_des, inv_sig_euler):
        ...
        s_phi, s_nu, c_phi, c_nu = np.sin(euler_angles[0]), np.sin(euler_angles[1]), np.cos(euler_angles[0]), np.cos(euler_angles[1])
        ...
        # print 'nll', nll
        # print '###########################'
        grad_nll = tmp1 + np.dot(jac_pos_th.T, tmp2) + np.dot(jac_ori_th.T, tmp3)
        return nll, grad_nll
    ...
    def inv_kin_ash_pose(self, mu_theta, sig_theta, mu_x, sig_x, mu_ang_euler_des, sig_euler):
        ...
        cost_grad = lambda theta: self.laplace_cost_and_grad_pose(theta, mu_theta, inv_sig_theta, mu_x, inv_sig_x, mu_ang_euler_des, inv_sig_euler)
        cost = lambda theta: cost_grad(theta)[0]
        grad = lambda theta: cost_grad(theta)[1]
    ...
    def laplace_cost_and_grad_pose_quaternion(self, theta, mu_theta, inv_sigma_theta, mu_x, inv_sigma_x, mu_quat_des, inv_sig_quat):
        diff1 = theta - mu_theta
        tmp1 = np.dot(inv_sigma_theta, diff1)
        ...
        quaternions = tf_tran.quaternion_from_matrix(f_th)
        diff3 = np.array(quaternions) - np.array(mu_quat_des)
        tmp3 = np.dot(inv_sig_quat, diff3)

        nll = 0.5*(np.dot(diff1, tmp1) + np.dot(diff2, tmp2) +  np.dot(diff3, tmp3))

        return nll

    def inv_kin_ash_pose_quaternion(self, mu_theta, sig_theta, mu_x, sig_x, mu_quat_des, sig_quat):
        inv_sig_theta = np.linalg.inv(sig_theta)
        inv_sig_x = np.linalg.inv(sig_x)
        inv_sig_quat = np.linalg.inv(sig_quat)
        cost_grad_quat = lambda theta: self.laplace_cost_and_grad_pose_quaternion(theta, mu_theta, inv_sig_theta, mu_x, inv_sig_x, mu_quat_des, inv_sig_quat)
        cost = lambda theta: cost_grad_quat(theta)
        res = opt.minimize(cost, mu_theta, method='BFGS')
        post_mean = res.x
        post_cov = res.hess_inv
        return post_mean, post_cov
...
if __name__ == '__main__':
    ...
    # #
    #
    # # Ash's Inverse Kinematics with orientation: Euler angles
    # mu_theta = current_joint_values
    # sig_theta = np.eye(7) * 0.1
    # mu_x = T_desired[0:3, -1]
    ...
    # post_mean_Ash, post_cov = franka_kin.inv_kin_ash_pose(mu_theta, sig_theta, mu_x, sig_x, mu_ang_euler_des, sig_euler)
    ...
    # Ash's Inverse Kinematics with orientation: Quaternions
    mu_theta = current_joint_values
    sig_theta = np.eye(7) * 0.1
    mu_x = T_desired[0:3, -1]
    sig_x = np.eye(3) * 0.000002
    mu_quat_des = tf_tran.quaternion_from_matrix(T_desired)
    sig_quat = np.eye(4) * 0.00002
    post_mean_Ash_quat, post_cov_quat = franka_kin.inv_kin_ash_pose_quaternion(mu_theta, sig_theta, mu_x, sig_x, mu_quat_des, sig_quat)
    T_invkinAshPose_quat, T_joint_invkinAshPose_quat = franka_kin.fwd_kin(post_mean_Ash_quat)
    franka_kin.plotter(ax, T_joint_invkinAshPose_quat, 'Inverse_kinematicsAshPose', color='magenta', )
