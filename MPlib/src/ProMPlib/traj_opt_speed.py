...
class TrajectoryOptimize(object):
    ...
    def __init__(self, nDoF, obstacle_list=None, ini_jv=None, des_jv=None,):
        ...
        self.lamda_smooth, self.lamda_obs = 2.0, 12.0

    def populate_obsacles(self,):
    ...
    def optimise(self, trajectory, withGrad=True):
        ...
        if withGrad:
            ...
            opti_traj = opt.minimize(self.calculate_total_cost, trajectory_flat, method='BFGS',
                                     jac=self.cost_gradient_analytic, options={'maxiter': 50, 'disp': True}, callback=self.optim_callback)

            cur = time.time()
            print 'CHOMP time with gradient:', cur - now

            opti_traj = np.transpose(opti_traj.x.reshape((self.nDoF, len(opti_traj.x) / 7)))
            opti_traj = np.insert(opti_traj, 0, self.initial_joint_values, axis=0)
            opti_traj = np.insert(opti_traj, len(opti_traj), self.desired_joint_values, axis=0)
            # #
        else:
            # ###################### Without Gradient ############################
            now = time.time()
            ...
            opti_traj = np.transpose(opti_traj.x.reshape((self.nDoF, -1)))
    ...
    def optim_callback(self, xk):
        ...
        self.__optCurve.append(xk)
        if self.debug:
            print 'Iteration {}: {}\n'.format(len(self.__optCurve), costs)
...
if __name__ == '__main__':
    ...
    initial_trajectory = traj_opt_original.discretize(initial_joint_values, desired_joint_values, step_size=0.3)
    ...
    optimised_trajectory = traj_opt.optimise(initial_trajectory,)
